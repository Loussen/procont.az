    ...
    <div class="container footer-top">
        <div class="row gy-4">
            <div class="col-lg-4 col-md-6 footer-about">
                ...
                <div class="social-links d-flex mt-4">
                    ...
                    @foreach($siteSettings->social_profiles as $socialProfile)
                        <a href="{{ $socialProfile['link'] }}" target="_blank"><i
                                    class="bi bi-{{ $socialProfile['social_network'] }}"></i></a>
                    @endforeach
                </div>
            </div>
            ...
            <div class="col-lg-2 col-md-3 footer-links">
                ...
                <ul>
                    @foreach(\App\Models\Services::all() as $service)
                        <li>
                            <a href="{{ route('service',['id' => $service->id,'locale' => \Illuminate\Support\Facades\App::getLocale()]) }}">{{ $service->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="col-lg-2 col-md-3 footer-links">
                <h4>Hospitals</h4>
                <ul>
                    @foreach(\App\Models\Products::all() as $hospital)
                        <li>
                            <a href="{{ route('hospital',['id' => $hospital->id,'locale' => \Illuminate\Support\Facades\App::getLocale()]) }}">{{ $hospital->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="col-lg-2 col-md-3 footer-links">
                ...
                <ul>
                    @foreach(\App\Models\Departments::all() as $department)
                        <li>
                            <a href="{{ route('department',['id' => $department->id,'locale' => \Illuminate\Support\Facades\App::getLocale()]) }}">{{ $department->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
